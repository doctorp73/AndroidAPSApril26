...
/**
 * Converts an AAPS basal schedule/profile into the pod's 30-minute-slot [BasalProgram]
 * wire format. Duplicated from [app.aaps.pump.omnipod.dash.util.mapProfileToBasalProgram]
 * (parametrized on [PumpType] instead of hardcoding [PumpType.OMNIPOD_DASH]) rather than
 * shared, because `dash` depends on `common`, not the reverse - this module can't import
 * from `dash`.
 */
        ...
        if (previousBasalValue != null) {
            entries.add(
                BasalProgram.Segment(
                    ...
                    startSlotIndex,
                    (pumpType.determineCorrectBasalSize(previousBasalValue.value) * 100).roundToInt()
                )
            )
        }
