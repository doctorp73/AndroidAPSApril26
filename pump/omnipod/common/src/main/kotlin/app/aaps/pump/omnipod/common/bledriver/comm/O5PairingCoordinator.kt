...
class O5PairingCoordinator @Inject constructor(
    ...
    fun pairNewPod(): PairResult {
        ...
        // The pod's real id isn't known before pairing; use the same predicted-id scheme
        // Dash's own Ids/LTKExchanger use (myId's address, rotated - see Id.increment()'s
        // doc comment), NOT the "not yet activated" placeholder - that placeholder is only
        // the BLE message *destination* address during pairing (see O5LTKExchanger's
        // internal `podAddress` field), a distinct concept from this predicted `podId`.
        ...
        if (resyncSqn != null) {
            // Matches Dash's own pairNewPod()->connect() resynchronization handling: a
            // fresh pairing needing EAP-AKA resync on its very first session establishment
            // would be unusual, but handle it the same way Dash does rather than assuming
            // it can't happen.
