...
import app.aaps.pump.omnipod.common.bledriver.pod.util.P256Codec
...
class O5CertificateStore(
    ...
    companion object {
        ...
        //
        // Hand-rolled rather than delegated to a library so it could be fully verified in
        // this environment against java.security.Signature's own DER output/input as ground
        // truth (round-trip + cross-verification tests), without depending on being able to
        // compile-check against an external ASN.1 library here.
        ...
        private fun encodeDerInteger(value: BigInteger): ByteArray {
            // BigInteger.toByteArray() already produces the minimal two's-complement
            // encoding, which for a non-negative value is exactly DER INTEGER's content
            // encoding (including the leading 0x00 pad byte when the magnitude's high bit
        ...
        // -- DER Certificate Key Extraction ----------------------------------------------

        /// The fixed DER header bytes for a P-256 SubjectPublicKeyInfo + uncompressed point
        /// indicator: SEQUENCE(89) > SEQUENCE(19) > OID(ecPublicKey) + OID(secp256r1) >
        /// BIT STRING(66) > 0x00 > 0x04
        private val p256SpkiHeader = byteArrayOf(
            ...
            0x06, 0x08, 0x2a, 0x86.toByte(), 0x48, 0xce.toByte(), 0x3d, 0x03, 0x01, 0x07, // OID secp256r1
        ...
        // -- DER Certificate Field Extraction ---------------------------------------------
        ...
        fun extractSerialNumber(certDER: ByteArray): ByteArray? {
            // Try v3 first: [0] EXPLICIT { INTEGER 2 } = a0 03 02 01 02, followed by the
            // serial itself: 02 [length] [serial bytes].
            ...
            // Parse the outer SEQUENCE, then the inner TBSCertificate SEQUENCE, then read the
            // serial INTEGER directly.
        ...
        // -- shared DER length/integer parsing --------------------------------------------
        ...
        private fun readDerInteger(data: ByteArray, offset: Int): Pair<BigInteger, Int> {
            ...
            // DER INTEGER content is already a signed two's-complement encoding; ECDSA's
            // r and s are always non-negative, so this is equivalent to BigInteger(1, bytes)
            // whenever the encoding is well-formed, but using the signed constructor here
            // means a malformed/negative value surfaces as a wrong (negative) BigInteger
            // rather than silently being reinterpreted as positive.
