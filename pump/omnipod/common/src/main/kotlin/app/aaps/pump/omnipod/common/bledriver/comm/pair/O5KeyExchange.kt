...
class O5KeyExchange(
    ...
    fun getSPSNonce(direction: Direction): ByteArray {
        if (pdmNonce.size < 6 || podNonce.size < 6) {
            return ByteArray(0) // will cause AES-CCM to fail with a clear error, matching Swift
        }
    ...
    private fun incrementNonceInPlace(nonce: ByteArray): ByteArray {
        ...
        val incremented = counter + 1 // Long wraps on overflow the same way a UInt64 bit pattern does
