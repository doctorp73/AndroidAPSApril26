...
class O5LTKExchanger(
    ...
    // -- SP2: the embedded classic-format "get pod status" probe -----------------------
    //
    // SP2 = [4-byte podId][0x00][0x03][0x0e 0x01 0x00][2-byte CRC], where [0x0e 0x01 0x00]
    // is a type-0 GetStatus command (commandType=0x0e, bodyLen=1, statusResponseType=0),
    // and the whole thing is wrapped in the classic (pre-BLE, RF-era) Omnipod message
    // envelope: address(4, BE) || b9 || bodyLen || body || crc16(2), b9 encoding
    // sequenceNum=0 and expectFollowOnMessage=false (b9 = 0x00 here).
    ...
    // -- SPS2.1 (certificate confirmation, short path: cert only, no signature) -----------
    ...
    // -- AES-CCM helpers (tag=8 bytes, no associated data) ----------------------------------
    //
    // Uses the same SpongyCastle CCMBlockCipher+AESEngine primitives, and the same
    // single-call processPacket pattern, as the existing app.aaps.pump.omnipod.common
    // .bledriver.comm.endecrypt.EnDecrypt — the only difference is O5 uses no associated
    // (header) data, where EnDecrypt's Dash session encryption uses a 16-byte header as AAD.
