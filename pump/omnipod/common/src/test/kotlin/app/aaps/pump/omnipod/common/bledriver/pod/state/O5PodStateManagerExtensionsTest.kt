...
class O5PodStateManagerExtensionsTest : TestBase() {
    ...
    // -- basalPulsesDelivered / basalDelivered / basalDrift ---------------------------------
    ...
    fun `basalPulsesDelivered and basalDelivered subtract bolus pulses from the pod total`() {
        ...
        assertThat(state.basalDelivered).isWithin(1e-9).of(4.0) // 80 * 0.05U
    }
    ...
    fun `basalDrift is positive when more basal was delivered than expected`() {
        ...
        assertThat(state.basalDrift).isWithin(1e-9).of(1.0) // delivered 4.0U, expected 3.0U
    }
    ...
    fun `basalDrift is negative when less basal was delivered than expected`() {
        ...
        assertThat(state.basalDrift).isWithin(1e-9).of(-1.0) // delivered 4.0U, expected 5.0U
    }
    ...
    fun `integrateO5ExpectedDelivery blends temp basal and scheduled basal across a boundary`() {
        val now = System.currentTimeMillis()
        // Temp basal (2.0U/h) covers only the first 30 minutes of a 1-hour window; the
        // scheduled program (1.0U/h) applies to the remaining 30 minutes.
        val result = integrateO5ExpectedDelivery(
            now, now + 3600_000L,
        ...
        assertThat(requireNotNull(result)).isWithin(1e-6).of(1.5) // 2.0*0.5 + 1.0*0.5
    }
    ...
    fun `nextBasalExpected seeds itself from basalDelivered the first time activation is complete`() {
        ...
        assertThat(requireNotNull(result)).isWithin(1e-9).of(5.0) // 100 * 0.05U
    }
    ...
    fun `nextBasalExpected integrates the elapsed period onto the existing value`() {
        ...
        assertThat(requireNotNull(result)).isWithin(1e-6).of(3.0) // 2.0 + 1.0U integrated over the hour
    }
}
