...
internal class O5OmnipodWizardViewModelTest {
    ...
    fun everyWizardStep_hasATitleAndTextResourceMapping() {
        // getTitleForStep/getTextForStep are non-exhaustive `when` blocks over an enum with
        // no `else` branch - a missing case would be a compile error, but this locks down
        // that every step really does resolve to *some* non-crashing resource id at runtime too.
        for (step in OmnipodWizardStep.entries) {
            sut.getTitleForStep(step) // must not throw
            sut.getTextForStep(step) // must not throw
        }
    }
