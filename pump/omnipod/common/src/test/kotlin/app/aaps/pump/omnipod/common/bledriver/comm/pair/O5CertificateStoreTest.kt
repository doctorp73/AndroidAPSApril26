...
class O5CertificateStoreTest {
    ...
    fun `verifySignature accepts a raw 64-byte signature directly, not just DER`() {
        // signRaw already returns raw (r||s) form - this asserts verifySignature's
        // DER-first-then-raw-fallback logic actually reaches the raw path.
    ...
    fun `extractSerialNumber falls back to v1 parsing when there is no version tag`() {
        ...
        // outer SEQUENCE(6) { inner TBSCertificate SEQUENCE(4) { INTEGER(2) [serial] } }
